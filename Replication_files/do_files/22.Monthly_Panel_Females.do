...
foreach c of numlist 1950/2002 {
	...
	*Ajuste mensual a 30 días:
	...
	drop sim? sim?? W*
	qui compress
	*order daysworked simultaneous, after(days) 
	*order episodesmonth, first 
	...
	* Añadir al sector público aquellos que tengan firmtype = P, Q, S
	...
	save  "${path}\dta)files\females_2018_`c'", replace
...
use ${path}\dta_files\females_2018_1950, clear
set more off
foreach c of numlist 1951/1964{
qui	append using ${path}\dta_files\females_2018_`c'
}	
*drop if year<1980
qui compress
sort pid month
save ${path}\dta_files\m2018_bboomers_females.dta, replace

use ${path}\dta_files\females_2018_1965, clear
set more off
...
qui compress
sort pid month
save ${path}\dta_files\m2018_genx_females.dta, replace

use ${path}\dta_files\females_2018_1981, clear
set more off
foreach c of numlist 1982/2002{
qui	append using ${path}\dta\females_2018_`c'
}	
*drop if year<1980
qui compress
sort pid month
save ${path}\dta_files\m2018_millennials_females.dta, replace

forvalues c=1950/2002 {
erase ${path}\dta_files\females_2018_`c'.dta
}
